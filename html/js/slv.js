...
var config = {
    "service": function () {
        return document.querySelector('input[name="archive"]:checked').value;
    },
    "direction": function () {
        return document.querySelector('input[name="direction"]:checked').value;
    },
    "re": function () {
        return document.getElementById("re").value;
    },
    "context": function () {
...
function next_archive() {
    ...
    return archive[(checked + 1) % archive.length].value;
}
function yesterday() {
    state.today += 1;
...
function scroll_by_page(direction) {
    var pager = document.getElementById("pager");
...
function changed(setting, opt) {
    ...
    switch (t) {
        ...
        case "non_neg_integer":
            var id = document.getElementById(setting);
            var v = id.value;
            // @ts-ignore
...
function set_from_query_string(query) {
    if (!query)
        return;
    var p = query.split('&');
...
function main() {
    ...
    ctl.on("swipeleft", function (ev) {
        open(tomorrow());
    });
    ctl.on("swiperight", function (ev) {
        open(yesterday());
    });
    ctl.on("swipedown", function (ev) {
        scroll({ "tail": function () {
                if (window.scrollY == 0) {
                    send("next", lines());
                }
    ...
    ctl.on("doubletap", function (ev) { fontsize(); });
    status.on("doubletap", function (ev) { fontsize(); });
    ctl.on("press", function (ev) {
        document.getElementById(state.tab).click();
        settings.style.display = "block";
    });
    status.on("press", function (ev) {
        document.getElementById(state.tab).click();
        settings.style.display = "block";
    });
